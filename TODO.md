...
## 17. Test Duration Tracking & Named Timers
...
## 20. Improved Drag-and-Drop Animations
...
## 22. Test Cleanup & Artifact Management
...
## 23. Fluent Assertions
Add richer validation steps that read like fluent assertions. Support natural language-style conditions such as `"x" should be "5"`, `"x" should be equal to "y"`, `"x" should contain "hello"`, `"x" should be greater than 10`. These would work with test variables (item 6) and model-extracted values to enable data-driven verification beyond just visual element presence.
...
## 25. Prevent Sleep During Test Runs
Suppress the Windows sleep/screen-off timer while a test is running. Use `SetThreadExecutionState(ES_CONTINUOUS | ES_SYSTEM_REQUIRED | ES_DISPLAY_REQUIRED)` at the start of a run and reset it when the run completes (or is cancelled/fails). A locked or sleeping screen would break screenshots and input, silently failing the entire test.
